...
INSOLID_TYPE Polyhedra :: BoxInSolid (const BoxSphere<3> & box) const
{
  ...
  return PointInSolid (box.Center(), 1e-3f * box.Diam());
}
				       ...
				       double eps) const
{
  ...
  cout << "primitive::vecinsolid2 makes nonsense for polyhedra" << endl;
...
{
  cout << "polyhedra, add face " << pi1 << ", " << pi2 << ", " << pi3 << endl;
...
{
  const double eps = 1e-10f*poly_bbox.Diam();
