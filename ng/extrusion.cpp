...
namespace netgen
{
				   ...
				   int seg) const
  {
	...
	double l = Dist(line_path[seg]->StartPI(),
			line_path[seg]->EndPI());
	t = min2(max2((point3d - line_path[seg]->StartPI()) * y_dir[seg],0.0),
		 l);	
  ...
  void ExtrusionFace :: CalcGradient (const Point<3> & point, Vec<3> & grad) const
  {
    ...
    double dFdxbar = 2.0*profile_spline_coeff(0)*p2d(0) +
      profile_spline_coeff(2)*p2d(1) + profile_spline_coeff(3);

    double dFdybar = 2.0*profile_spline_coeff(1)*p2d(1) +
  ...
  void ExtrusionFace :: CalcHesse (const Point<3> & point, Mat<3> & hesse) const
  {
	...
	auxvec = (1.0/(2.0*eps)) * (auxgrad2-auxgrad1);
	for(int j=0; j<3; j++)
	  hesse(i,j) = auxvec(j);
	auxpoint1(i) = point(i);
	auxpoint2(i) = point(i);
  ...
  double ExtrusionFace :: HesseNorm () const
  {
    return fabs(profile_spline_coeff(0) + profile_spline_coeff(1)) +
      sqrt(pow(profile_spline_coeff(0)+profile_spline_coeff(1),2)+4.0*pow(profile_spline_coeff(2),2));
  }
