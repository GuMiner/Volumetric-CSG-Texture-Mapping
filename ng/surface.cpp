...
VecInSolid2 (const Point<3> & p,
	     ...
	     double eps) const
{
  //cout << "Primitive::VecInSolid2" << endl;
  Point<3> hp = p + 1e-3 * v1 + 1e-5 * v2;

  INSOLID_TYPE res = PointInSolid (hp, eps);
  //  cout << "vectorin2, type = " << typeid(*this).name() << ", res = " << res << endl;
...
VecInSolid3 (const Point<3> & p,
	     ...
	     double eps) const
{
  //cout << "Primitive::VecInSolid3" << endl;
...
VecInSolid3 (const Point<3> & p, const Vec<3> & v, const Vec<3> & v2,
	     double eps) const
{
  //cout << "OneSurfacePrimitive::VecInSolid3" << endl;
  double hv1 = (GetSurface(0).CalcFunctionValue(p));
  if (hv1 <= -eps)
    return IS_INSIDE;
  if (hv1 >= eps)
